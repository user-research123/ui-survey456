...
def update_html_report():
    ...
    if target_marker in html_content:
        ...
        html_content = html_content[:insert_pos] + new_weibo_html + '\n\n' + html_content[insert_pos:]
        print("✓ 已添加 4 月 8 日微博舆情分析内容到 HTML 报告")
    else:
        print("✗ 未找到 4 月 7 日内容块，无法插入")
        print(f"查找目标：{target_marker}")
        return False
    
    # 更新日期切换按钮 - 在 4 月 7 日按钮前添加 4 月 8 日按钮
    # 先将原来的 active 按钮改为非 active
    old_button = '<button class="date-tab active" onclick="showUserFeedbackDate(\'04-07\')">4 月 7 日</button>'
    new_button_inactive = '<button class="date-tab " onclick="showUserFeedbackDate(\'04-07\')">4 月 7 日</button>'
    html_content = html_content.replace(old_button, new_button_inactive)
    
    # 然后插入新的 active 按钮
    new_button_active = '<button class="date-tab active" onclick="showUserFeedbackDate(\'04-08\')">4 月 8 日</button>\n                    '
    html_content = html_content.replace(new_button_inactive, new_button_active + new_button_inactive)
