...
if len(unique_dates) > 2:
    ...
    # 使用二次多项式拟合实现平滑效果
    coeffs = np.polyfit(date_nums, unique_ranks, deg=2)
    poly_func = np.poly1d(coeffs)
    
    # 生成更密集的x轴点用于绘制平滑曲线
    smooth_date_nums = np.linspace(min(date_nums), max(date_nums), 300)
    smooth_ranks = poly_func(smooth_date_nums)
    
    # 确保排名不为负数且在合理范围内
    smooth_ranks = np.clip(smooth_ranks, 0, max(unique_ranks) + 5)
    
    # 绘制平滑曲线（主曲线）
    ax.plot(mdates.num2date(smooth_date_nums), smooth_ranks, linewidth=2.5, color='#2E86AB', alpha=0.8)
    
    # 绘制原始数据点
    ax.scatter(unique_dates, unique_ranks, s=60, c='#2E86AB', zorder=5, edgecolors='white', linewidth=2)
    
    # 添加填充区域（曲线下方）
    ax.fill_between(mdates.num2date(smooth_date_nums), smooth_ranks, max(unique_ranks) + 5, alpha=0.1, color='#2E86AB')
else:
    # 数据点太少时直接绘制折线
    ax.plot(unique_dates, unique_ranks, marker='o', markersize=6, linewidth=2, color='#2E86AB')

# 设置Y轴范围，从最大排名+5到0（0在最上面）
max_rank = max(unique_ranks)
ax.set_ylim(max_rank + 5, 0)

# 设置Y轴刻度为整数，从顶部0开始向下递增
