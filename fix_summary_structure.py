...
# 使用正则表达式匹配并替换总结内容
# 匹配从"D 月 07 日总结"或"4 月 7 日总结"开始到</div>结束的内容（包含可能的错误</p>标签）
pattern = r'(D 月 07 日总结|4 月 7 日总结)<br>.*?</div>\n                '

# 替换总结内容
new_content = re.sub(pattern, new_summary + '</div>\n                ', content, flags=re.DOTALL)

# 写回文件
with open(html_path, 'w', encoding='utf-8') as f:
    f.write(new_content)

print("✅ 总结模块 HTML 结构已修复")
print("💡 移除了错误的</p>标签，现在结构正确")
